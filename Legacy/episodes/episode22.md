---
id: legacy_episode22
aliases:
  - episode22
tags: []
---

# Episode 22:  Return to Rogueport and the next leg of adventure

## Characters

## Scenes
- Start out with the Egoluis and Jeanne in Two Rivers right before it get's consumed by `The Void`
- The party returns to Rogueport
- Battle versus the CoV
- The party heads to the find the next rod piece


## Locations
- Rogueport

## Secrets and Clues
- The `Void holes` have popped up all over the world and it's still unknown why.
- The `Red Hands` has mutiple HQs around the world 
- The "Covenant of the Sword" has finally tracked down `Tef` to try to get back the stolen, they are looking for it
- There is a power vac in Rogueport since the `Mirelock` HQ was sucked up into a `Void Rift` between differnt gang heads.
- Strange creatures are rummoered to come out of the portals.
